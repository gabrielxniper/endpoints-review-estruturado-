...
export class PostBusiness {
    ...
    postCreate = (title: string, content: string, authorId: number) : Post => {
        ...
        if (!content) { errors.push("O campo 'content' é obrigatório."); }
        if (!authorId) { errors.push("O campo 'authorId' é obrigatório."); }
        if (typeof title !== 'string' || title.length < 3) { errors.push("O título deve ser um texto com pelo menos 3 caracteres."); }
        if (typeof content !== 'string' || content.length < 10) { errors.push("O conteúdo deve ser um texto com pelo menos 10 caracteres."); }
        if (typeof authorId !== 'number') { errors.push("O campo 'authorId' deve ser um número."); }
    ...
    postUpdate = (id: any, dataToUpdate: any) => {
        ...
        for (let i = 0; i < forbiddenKeys.length; i++) {
            ...
            if (key in dataToUpdate) {
                throw new Error(`Não é permitido alterar o campo '${key}'.`);
            }
        }
    ...
    public deletePost = (postId: string, userId: string) => {
        ...
        if (!userRequesting) { throw new Error("Utilizador da requisição não encontrado."); }
